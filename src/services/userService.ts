...
export const USER_LIST_STORAGE_KEY = "@leanwork:users";

export const userService = {
  getUsers: () => {
    const rawUsers = localStorage.getItem(USER_LIST_STORAGE_KEY);
  ...
  editUser: (cpf: string, data: Partial<Omit<User, "cpf">>) => {
    ...
    localStorage.setItem(USER_LIST_STORAGE_KEY, JSON.stringify(editedUsers));
  },
  deleteUser: (cpf: string) => {
    const users = userService.getUsers();
    const newUsers = users.filter((user) => user.cpf !== cpf);
    localStorage.setItem(USER_LIST_STORAGE_KEY, JSON.stringify(newUsers));
    return newUsers;
  },
};
