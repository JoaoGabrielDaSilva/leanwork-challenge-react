...
const PROJECT_URL = "localhost:8080";

test.describe("User Register Screen", () => {
  test("Button should be disabled if fields are empty", async ({ page }) => {
    await page.goto(`${PROJECT_URL}/user-register`);

    await expect(page.getByText("Cadastrar")).toBeDisabled();
  });

  test("Button should be enabled when fields are filled", async ({ page }) => {
    await page.goto(`${PROJECT_URL}/user-register`);
    ...
    await page
      .getByLabel("Telefone")
      .fill(faker.string.numeric({ length: 13 }));

    await expect(page.getByText("Cadastrar")).toBeEnabled();
  });

  test("Errors should be shown if fields are invalid", async ({ page }) => {
    await page.goto(`${PROJECT_URL}/user-register`);
  ...
  test("It should navigate to user list page", async ({ page }) => {
    await page.goto(`${PROJECT_URL}/user-register`);
